...
### 7.4. ตาราง `debt_trn` และ `debt_trn_pivot` (ตารางข้อมูลหนี้ค้างชำระ)
* **ปัจจุบันยังไม่ส่งผลกระทบใด ๆ ต่อตารางอื่น**: โครงสร้างและข้อมูลของตารางถูกสร้างไว้ในฐานข้อมูลเรียบร้อยแล้ว แต่ระบบแอปพลิเคชันยังไม่ได้มีการเชื่อมโยงหรือเขียน API เพื่อดึงข้อมูลส่วนนี้ไปคำนวณหรือแสดงผลแต่อย่างใด

> [!IMPORTANT]  
> ข้อมูลสรุปผลในข้อ 7.1 ถึง 7.3 จะไม่อัปเดตบนหน้าแสดงผล Dashboard โดยอัตโนมัติเมื่อแก้ข้อมูลในฐานข้อมูล
> ทุกครั้งที่มีการเปลี่ยนแปลงตารางข้อมูลดิบเหล่านี้ **จะต้องทำการรันสคริปต์ `node migrate.js`** เพื่อบังคับให้ระบบกวาดข้อมูลทั้งหมดมาคำนวณและอัปเดตลงตารางแสดงผลใหม่ทุกครั้ง
